...
function getZoneColor(zone: number): string {
  const colors = [
    ...
    "rgb(255, 150, 150)", // 2 - средне-красн��й
    ...
    switch (language) {
      ...
      default:
        return {
          ...
          cost: "Стоимость: 1% (без учета НДС 12%)",
          description:
            "Дополнительный платеж, уплачиваемый Заказчиком Исполнителю при оглашении стоимости почтового отправления. Расчет размера дополнительного платежа производится по формуле - дополнительный платеж от стоимости (суммы) = 1% x об��явленная стоимость (сумма).",
        };
    }
  };
      ...
      case "ru":
        return {
          ...
          rates: [
            ...
            "от 100 кг ��о 499 кг – каждый килограмм по 4000 сум",
            ...
            <table className="border-collapse text-xs lg:text-sm w-full min-w-[800px]">
              ...
              <tbody>
                {zoneMatrix.map((row, rowIndex) => (
                  <tr key={rowIndex}>
                    <td className="border border-gray-300 px-1 lg:px-2 py-2 text-center font-medium bg-white text-xs lg:text-sm sticky left-0 z-10 md:static min-w-[80px] lg:min-w-[100px]">
                      {cityNames[rowIndex]}
                    </td>
