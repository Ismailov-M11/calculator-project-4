    ...
    switch (language) {
      ...
      default:
        return {
          ...
          cost: "Стоимость: 1% (без учета НДС 12%)",
          description:
            "Дополнительный платеж, уплачиваемый Заказчиком Исполнителю при оглашении стоимости почтового отправления. Расчет размера дополнительного платежа производится по формуле - дополнительный платеж от стоимости (суммы) = 1% x объявлен��ая стоимость (сумма).",
        };
    }
  };
  ...
  const getDeliveryRatesText = () => {
    switch (language) {
      case "ru":
        return {
          title:
            "Во всех действующих тарифах используется следующая логика подсчета стоимости дос��авки за каждый килограмм:",
          rates: [
    ...
    switch (language) {
      ...
      default:
        return {
          ...
          notes: [
            "Каждое Отправление тарифицируется по отдельности. В стоимость включена одна попытка доставки Отправления;",
            "Если получатель отсутствует по адресу д��ставки или не может принять Отправление по другим, не зависящим от курьера причинам, то каждый последующий приезд курьера тарифицируется как новая отдельная заявка на доставку;",
            "Если Получатель изменил адрес доставки и просит доставить по другому адресу, то перенаправление посылки по новому адресу, будет тарифицироваться как отдельная заявка на доставку;",
            "Вес одного места Отправления не должен превышать - 30 кг;",
            "Если при вызове курьера Отправитель откажется передавать Отправление к доставке, Заказчик должен оплатить за ложный вызов курьера 25 000 сум без НДС (28 000 с НДС)",
          ],
            ...
            <table className="w-full border-collapse min-w-[700px] lg:min-w-[900px] text-xs lg:text-sm">
              <thead>
                <tr className="bg-red-600 text-white">
                  <th className="border border-white px-1 py-2 text-xs lg:text-sm font-medium text-center sticky left-0 bg-red-600 z-10 md:static">
                    {getWeightHeader()}
                  </th>
              ...
              <tbody>
                {ratesData.map((row, index) => (
                  ...
                  >
                    <td className="border border-gray-300 px-1 py-2 text-center font-medium text-xs lg:text-sm sticky left-0 bg-inherit z-10 md:static">
                      {row.weight}
                    </td>
      ...
      <Card className="border-0 shadow-lg bg-white/90 backdrop-blur-sm">
        <CardContent className="p-3 lg:p-6 w-full">
          <div className="w-full overflow-x-auto">
            <table className="border-collapse text-xs lg:text-sm w-full min-w-[800px]">
              <thead>
                <tr>
